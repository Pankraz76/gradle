...
abstract class AbstractVisualStudioIntegrationSpec extends AbstractInstalledToolChainIntegrationSpec {
    ...
    void useMsbuildTool() {
        ...
        executor.requireIsolatedDaemons().requireDaemon()
        ...
        initScript << """
            ...
            class Helpers {
                static def redirectOutputForAll(nodes, relativeToRoot) {
                    nodes.each { node ->
                ...
                static def redirectOutput(Node node, String relativeToRoot) {
...
''' + value + ' 1>%outputLog% 2>%errorLog%'
    ...
    protected MSBuildExecutor getMsbuild() {
        ...
        def executor = new MSBuildExecutor(testDirectory, toolChain)
        executor.withArgument('/p:Platform=Win32')
        return executor
    }
