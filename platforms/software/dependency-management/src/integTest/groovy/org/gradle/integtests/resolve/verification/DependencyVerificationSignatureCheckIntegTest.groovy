...
This can indicate that a dependency has been compromised. Please carefully verify the signatures and checksums. Key servers are disabled, this can indicate that you need to update the local keyring with the missing keys."""
    ...
    def "ignores case for trusted keys"() {
        createMetadataFile {
            keyServer(keyServerFixture.uri)
            verifySignatures()
            addTrustedKey("org:foo:1.0", validPublicKeyHexString.toLowerCase().capitalize())
            addTrustedKey("org:foo:1.0", validPublicKeyHexString, "pom", "pom")
        }
    ...
    def "ignores case for ignored keys"() {
        createMetadataFile {
            keyServer(keyServerFixture.uri)
            verifySignatures()
            addGloballyIgnoredKey(getValidPublicKeyLongIdHexString().toLowerCase().capitalize())
        }
