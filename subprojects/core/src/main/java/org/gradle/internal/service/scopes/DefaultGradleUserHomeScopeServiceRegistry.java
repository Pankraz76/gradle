...
public class DefaultGradleUserHomeScopeServiceRegistry implements GradleUserHomeScopeServiceRegistry, Closeable {
    ...
    public void release(ServiceRegistry registry) {
        try {
            lock.lock();
            servicesForHomeDir.entrySet().stream()
                .filter(entry -> entry.getValue().registry == registry && entry.getValue().count > 0)
                .findFirst()
                .ifPresent(
                    entry -> {
                        if ((servicesForHomeDir.size() > 1 || getProperty(REUSE_USER_HOME_SERVICES, "true").equals("false"))
                            && entry.getValue().count == 1) {
                            stoppable(registry).stop(); // Other home dir in use, close these. Otherwise, keep the services for next time.
